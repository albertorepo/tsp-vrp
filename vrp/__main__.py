...
def main():
    ...
    cvrp = CVRPLocalSearch(distance_matrix=dists, demand=demand, truck_capacity=q, depot=0,
                           initial_solution_strategy='random')
    solution, cost = cvrp.run(max_iter=1000, verbose=100)
    print "Cost:", cost
    ...
    cvrp = CVRPLocalSearch(distance_matrix=dists, demand=demand, truck_capacity=q, depot=0, initial_solution=solution, neighborhood='2-opt')
